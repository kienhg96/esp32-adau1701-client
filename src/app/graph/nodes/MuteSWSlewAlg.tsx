 ...
 * Mute (with slew)
 */
function MuteSWSlewAlg({ node }: NodeContentProps) {
    ...
    function handleChange(checked: boolean) {
        ...
        if (address === undefined) {
            console.log("MuteSWSlewAlg: No address is found");
            return;
        }
        ...
        .catch(err => {
            console.error("MuteSWSlewAlg: Error submitting mute", err);
            setMute(prev);
        }).finally(() => {
            setSubmitting(false);
        });
...
MuteSWSlewAlg.displayName = "MuteSWSlewAlg";
export default MuteSWSlewAlg;
